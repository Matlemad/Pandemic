/**
 * iOS P2P Transport — Apple MultipeerConnectivity implementation
 * 
 * This module wraps the native MultipeerModule for iOS.
 */
...
const { MultipeerModule } = NativeModules;

/**
 * Check if the native module is available
 */
export const isMultipeerAvailable = 
  Platform.OS === 'ios' && MultipeerModule != null;

/**
 * iOS P2P Transport using Apple MultipeerConnectivity
 */
export class IOSP2PTransport extends BaseP2PTransport {
  ...
  constructor() {
    ...
    if (!isMultipeerAvailable) {
      console.warn('[IOSP2P] MultipeerModule not available');
      return;
    }
    
    this.nativeEventEmitter = new NativeEventEmitter(MultipeerModule);
  ...
  async initialize(config: P2PConfig): Promise<void> {
    if (!isMultipeerAvailable) {
      throw new P2PError(P2PErrorCode.NOT_SUPPORTED, 'MultipeerConnectivity not available');
    }

    this.config = { ...DEFAULT_P2P_CONFIG, ...config };
    this.displayName = config.displayName;

    await MultipeerModule.initialize({
      serviceId: this.config.serviceId,
      displayName: this.displayName,
    });

    // Generate local peer ID
    this.localPeerId = `ios-${Date.now()}`;
    
    console.log('[IOSP2P] Initialized');
  }
  ...
  async destroy(): Promise<void> {
    ...
    console.log('[IOSP2P] Destroyed');
  }

  async isAvailable(): Promise<boolean> {
    if (!isMultipeerAvailable) return false;
    return await MultipeerModule.isAvailable();
  }

  async requestPermissions(): Promise<boolean> {
    if (!isMultipeerAvailable) return false;
    
    const result = await MultipeerModule.checkPermissions();
  ...
  async startDiscovery(): Promise<void> {
    ...
    await MultipeerModule.startDiscovery();
  }

  async stopDiscovery(): Promise<void> {
    if (!isMultipeerAvailable) return;
    await MultipeerModule.stopDiscovery();
  }
  ...
  async startAdvertising(room: RoomAdvertisement): Promise<void> {
    ...
    await MultipeerModule.startAdvertising({
      roomId: room.roomId,
  ...
  async stopAdvertising(): Promise<void> {
    if (!isMultipeerAvailable) return;
    await MultipeerModule.stopAdvertising();
  }
  ...
  async connectToHost(hostPeerId: PeerId): Promise<void> {
    ...
    await MultipeerModule.connectToHost(hostPeerId);
  }

  async acceptConnection(peerId: PeerId): Promise<void> {
    if (!isMultipeerAvailable) return;
    await MultipeerModule.acceptConnection(peerId);
  }

  async rejectConnection(peerId: PeerId): Promise<void> {
    if (!isMultipeerAvailable) return;
    await MultipeerModule.rejectConnection(peerId);
  }

  async disconnectFromPeer(peerId: PeerId): Promise<void> {
    if (!isMultipeerAvailable) return;
    await MultipeerModule.disconnectFromPeer(peerId);
  }

  async disconnectAll(): Promise<void> {
    if (!isMultipeerAvailable) return;
    await MultipeerModule.disconnectAll();
  }
  ...
  async sendBytes(toPeerId: PeerId, data: string, typeTag: string): Promise<boolean> {
    if (!isMultipeerAvailable) return false;
    return await MultipeerModule.sendBytes(toPeerId, data, typeTag);
  }
  ...
  async sendFile(
    ...
    return await MultipeerModule.sendFile(toPeerId, fileUri, {
      fileId: meta.fileId,
  ...
  async cancelTransfer(transferId: TransferId): Promise<void> {
    if (!isMultipeerAvailable) return;
    await MultipeerModule.cancelTransfer(transferId);
  }
}

// Export singleton instance
export const iosP2PTransport = new IOSP2PTransport();
export default iosP2PTransport;
