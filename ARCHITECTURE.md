# 🏗️ PANDEMIC - Documentazione Architetturale

Questo documento descrive in dettaglio l'architettura, le decisioni di design e i protocolli utilizzati in Pandemic.

---

## 📋 Indice

1. [Risposte alle Domande di Design](#risposte-alle-domande-di-design)
2. [Diagramma Architetturale](#diagramma-architetturale)
3. [Matrice Decisionale BLE vs Wi-Fi LAN](#matrice-decisionale)
4. [Protocollo di Comunicazione](#protocollo-di-comunicazione)
5. [Formati dei Messaggi](#formati-dei-messaggi)
6. [Gestione degli Edge Case](#gestione-degli-edge-case)
7. [Limitazioni di Piattaforma](#limitazioni-di-piattaforma)

---

## 1. Risposte alle Domande di Design

### 1.1 Qual è il ruolo della Room?

**Risposta: (a) Index + Coordinator**

La Room è un **coordinatore e indice**, NON un ponte dati. Ecco il modello:

```
┌─────────────────────────────────────────────────────────────────┐
│                          ROOM (HOST)                            │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                    RESPONSABILITÀ                        │   │
│  │  ✓ Mantenere lista dei peer connessi                    │   │
│  │  ✓ Aggregare metadati file condivisi                    │   │
│  │  ✓ Facilitare le presentazioni tra peer                 │   │
│  │  ✓ Broadcast aggiornamenti indice                       │   │
│  │                                                          │   │
│  │  ✗ NON memorizzare file audio                           │   │
│  │  ✗ NON fare proxy di trasferimenti                      │   │
│  │  ✗ NON agire come cache centrale                        │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  Mental Model:                                                  │
│  "La room presenta le persone. Loro si scambiano file          │
│   privatamente."                                                │
└─────────────────────────────────────────────────────────────────┘
```

**Flusso tipico:**
1. Guest A vuole scaricare file da Guest B
2. Guest A chiede alla Room l'indirizzo di Guest B
3. Room risponde con indirizzo LAN di Guest B
4. Guest A si connette DIRETTAMENTE a Guest B
5. Trasferimento avviene peer-to-peer
6. Room non vede mai i dati del file

### 1.2 WebRTC è adatto qui?

**Risposta: NO, WebRTC NON è adatto**

WebRTC richiede:
- ❌ **STUN servers** per scoprire l'IP pubblico
- ❌ **TURN servers** per relay quando P2P fallisce
- ❌ **Signaling server** per scambio SDP
- ❌ **IP networking** (non funziona su BLE)

Il nostro scenario:
- ✅ Nessun server Internet
- ✅ Solo rete locale
- ✅ BLE per discovery
- ✅ Wi-Fi LAN per trasferimento

**Alternative corrette:**
- HTTP streaming su LAN
- WebSocket su LAN
- TCP/UDP custom su LAN
- BLE GATT per fallback

### 1.3 Qual è il protocollo di trasferimento esatto?

**Path Primario: HTTP over Wi-Fi LAN**

```
┌─────────────────────────────────────────────────────────────────┐
│                   PROTOCOLLO DI TRASFERIMENTO                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  MODE: Wi-Fi LAN (Primario)                                    │
│  ─────────────────────────────────────────────────────────────  │
│                                                                 │
│  Requester                           Owner                      │
│  ─────────                           ─────                      │
│                                                                 │
│  1. GET /files/{fileId}                                         │
│     Headers:                                                    │
│       - X-Session-Token: {token}                               │
│       - Range: bytes=0- (opzionale)                            │
│                                   ────────────────────────────► │
│                                                                 │
│                                   2. Verifica token            │
│                                   3. Streaming risposta:       │
│                                      - 200 OK                  │
│                                      - Content-Type: audio/*   │
│                                      - Content-Length: {size}  │
│                                      - Transfer-Encoding:      │
│                                        chunked                 │
│  ◄────────────────────────────────                             │
│                                                                 │
│  4. Riceve chunks                                              │
│  5. Scrive su filesystem                                       │
│  6. Aggiorna progresso UI                                      │
│  7. Notifica completamento a Room                              │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  MODE: BLE Only (Fallback)                                     │
│  ─────────────────────────────────────────────────────────────  │
│                                                                 │
│  Caratteristiche:                                              │
│  • Chunk size: 512 bytes (MTU-dependent)                       │
│  • Throughput: ~50-200 KB/s                                    │
│  • ACK ogni 8 chunks                                           │
│  • Retry su timeout                                            │
│  • Checksum finale                                             │
│                                                                 │
│  Protocollo:                                                   │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  [SEQ:4][FLAGS:1][DATA:507]                             │   │
│  │                                                          │   │
│  │  FLAGS:                                                  │   │
│  │    0x01 = START (primo chunk)                           │   │
│  │    0x02 = END (ultimo chunk)                            │   │
│  │    0x04 = ACK_REQUEST (richiede conferma)               │   │
│  │    0x08 = RETRY (ritrasmissione)                        │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 2. Diagramma Architetturale

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         PANDEMIC - ARCHITETTURA                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│                              ┌─────────────┐                                │
│                              │   VENUE     │                                │
│                              │  (LUOGO)    │                                │
│                              └──────┬──────┘                                │
│                                     │                                       │
│          ┌──────────────────────────┼──────────────────────────┐           │
│          │                          │                          │           │
│          ▼                          ▼                          ▼           │
│   ┌─────────────┐            ┌─────────────┐           ┌─────────────┐     │
│   │   DEVICE A  │    BLE     │   DEVICE B  │    BLE    │   DEVICE C  │     │
│   │   (HOST)    │◄──────────►│   (GUEST)   │◄─────────►│   (GUEST)   │     │
│   └──────┬──────┘            └──────┬──────┘           └──────┬──────┘     │
│          │                          │                          │           │
│          │     ┌────────────────────┼────────────────────┐    │           │
│          │     │    LOCAL WI-FI NETWORK (No Internet)    │    │           │
│          │     │         192.168.x.x / 10.0.x.x          │    │           │
│          │     └────────────────────┼────────────────────┘    │           │
│          │                          │                          │           │
│          └──────────────────────────┼──────────────────────────┘           │
│                                     │                                       │
│                          ┌──────────▼──────────┐                           │
│                          │   DIRECT P2P        │                           │
│                          │   HTTP/WebSocket    │                           │
│                          │   FILE TRANSFER     │                           │
│                          └─────────────────────┘                           │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                             DEVICE ARCHITECTURE                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │                          UI LAYER                                   │    │
│  │                                                                     │    │
│  │  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────────┐  │    │
│  │  │  Home   │ │  Host   │ │  Join   │ │  Room   │ │   Library   │  │    │
│  │  │ Screen  │ │ Screen  │ │ Screen  │ │ Screen  │ │   Screen    │  │    │
│  │  └────┬────┘ └────┬────┘ └────┬────┘ └────┬────┘ └──────┬──────┘  │    │
│  └───────┼──────────┼──────────┼──────────┼──────────────┼───────────┘    │
│          │          │          │          │              │                 │
│  ┌───────▼──────────▼──────────▼──────────▼──────────────▼───────────┐    │
│  │                        STATE LAYER (Zustand)                       │    │
│  │                                                                     │    │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────────┐    │    │
│  │  │  appStore   │  │  roomStore  │  │     transferStore       │    │    │
│  │  │ ─────────── │  │ ─────────── │  │ ─────────────────────── │    │    │
│  │  │ deviceId    │  │ room        │  │ transfers[]             │    │    │
│  │  │ deviceName  │  │ role        │  │ activeCount             │    │    │
│  │  │ settings    │  │ peers[]     │  │ progress                │    │    │
│  │  │ networkCaps │  │ files[]     │  └─────────────────────────┘    │    │
│  │  └─────────────┘  └─────────────┘                                  │    │
│  └───────────────────────────────────────────────────────────────────┘    │
│          │                                                                  │
│  ┌───────▼──────────────────────────────────────────────────────────┐     │
│  │                       SERVICE LAYER                               │     │
│  │                                                                    │     │
│  │  ┌──────────────┐ ┌──────────────┐ ┌──────────────────────────┐  │     │
│  │  │  RoomService │ │ AudioLibrary │ │     TransferService      │  │     │
│  │  │              │ │   Service    │ │                          │  │     │
│  │  │ • createRoom │ │ • scanFiles  │ │ • downloadFile           │  │     │
│  │  │ • joinRoom   │ │ • importFile │ │ • uploadFile             │  │     │
│  │  │ • leaveRoom  │ │ • getMetadata│ │ • trackProgress          │  │     │
│  │  │ • shareFiles │ │ • deleteFile │ │ • handleErrors           │  │     │
│  │  └──────────────┘ └──────────────┘ └──────────────────────────┘  │     │
│  └───────────────────────────────────────────────────────────────────┘     │
│          │                                                                  │
│  ┌───────▼──────────────────────────────────────────────────────────┐     │
│  │                      TRANSPORT LAYER                              │     │
│  │                                                                    │     │
│  │  ┌────────────────────────┐  ┌────────────────────────────────┐  │     │
│  │  │      BLE Service       │  │       Network Service          │  │     │
│  │  │ ────────────────────── │  │ ────────────────────────────── │  │     │
│  │  │ • BLE advertising      │  │ • HTTP server (host)           │  │     │
│  │  │ • BLE scanning         │  │ • HTTP client (guest)          │  │     │
│  │  │ • GATT characteristics │  │ • WebSocket coordination       │  │     │
│  │  │ • Room discovery       │  │ • File streaming               │  │     │
│  │  │ • Handshake            │  │ • Network state detection      │  │     │
│  │  └────────────────────────┘  └────────────────────────────────┘  │     │
│  └───────────────────────────────────────────────────────────────────┘     │
│          │                                                                  │
│  ┌───────▼──────────────────────────────────────────────────────────┐     │
│  │                       STORAGE LAYER                               │     │
│  │                                                                    │     │
│  │  ┌────────────────────────┐  ┌────────────────────────────────┐  │     │
│  │  │    AsyncStorage        │  │       FileSystem               │  │     │
│  │  │ ────────────────────── │  │ ────────────────────────────── │  │     │
│  │  │ • Device settings      │  │ • Audio files                  │  │     │
│  │  │ • Cached metadata      │  │ • Imported files               │  │     │
│  │  │ • Device identity      │  │ • Downloaded files             │  │     │
│  │  └────────────────────────┘  └────────────────────────────────┘  │     │
│  └───────────────────────────────────────────────────────────────────┘     │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 3. Matrice Decisionale

### BLE vs Wi-Fi LAN

| Criterio | BLE | Wi-Fi LAN | Vincitore | Note |
|----------|-----|-----------|-----------|------|
| **Disponibilità** | Sempre | Richiede rete | BLE | BLE non richiede infrastruttura |
| **Discovery** | Eccellente | Richiede mDNS | BLE | BLE broadcast nativo |
| **Throughput** | 50-200 KB/s | 10-100+ Mbps | Wi-Fi | 100x+ differenza |
| **Range** | 10-30m | 50-100m | Wi-Fi | Dipende da ambiente |
| **Consumo batteria (idle)** | Molto basso | Basso | BLE | BLE ottimizzato |
| **Consumo batteria (transfer)** | Alto (lungo) | Medio (veloce) | Wi-Fi | Wi-Fi finisce prima |
| **Background iOS** | Molto limitato | Limitato | Pari | Entrambi problematici |
| **Setup utente** | Zero | Connetti a WiFi | BLE | BLE automatico |
| **File > 5MB** | Impraticabile | Ottimo | Wi-Fi | BLE troppo lento |
| **Affidabilità** | Buona | Eccellente | Wi-Fi | Wi-Fi più stabile |

### Decisione Finale

```
┌─────────────────────────────────────────────────────────────────┐
│                    DECISIONE ARCHITETTURALE                     │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                     BLE (Bluetooth)                      │   │
│  │                                                          │   │
│  │  Usato per:                                              │   │
│  │  ✓ Discovery stanze                                      │   │
│  │  ✓ Handshake iniziale                                    │   │
│  │  ✓ Scambio metadati piccoli (< 1KB)                     │   │
│  │  ✓ Fallback mode (file piccoli, emergenza)              │   │
│  │                                                          │   │
│  │  NON usato per:                                          │   │
│  │  ✗ Trasferimento file audio                              │   │
│  │  ✗ Streaming dati                                        │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                     Wi-Fi LAN                            │   │
│  │                                                          │   │
│  │  Usato per:                                              │   │
│  │  ✓ Trasferimento file audio                              │   │
│  │  ✓ Coordinazione real-time (WebSocket)                  │   │
│  │  ✓ Streaming grandi volumi dati                         │   │
│  │                                                          │   │
│  │  Prerequisito:                                           │   │
│  │  • Stessa rete Wi-Fi locale                              │   │
│  │  • No Internet richiesto                                 │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 4. Protocollo di Comunicazione

### 4.1 Flusso Completo

```
════════════════════════════════════════════════════════════════════════════════
                         PANDEMIC PROTOCOL v1.0
════════════════════════════════════════════════════════════════════════════════

PHASE 1: ROOM CREATION (HOST)
────────────────────────────────────────────────────────────────────────────────

   HOST DEVICE
   ┌────────────────────────────────────────────────────────────────────┐
   │                                                                    │
   │  1. User action: "Create Room"                                    │
   │                                                                    │
   │  2. Generate identifiers:                                         │
   │     • roomId: UUID v4                                             │
   │     • hostId: UUID v4 (persisted)                                 │
   │     • sessionKey: random 32 bytes                                 │
   │                                                                    │
   │  3. Detect network:                                               │
   │     • Check Wi-Fi connection                                      │
   │     • Get local IP (if available)                                 │
   │     • Determine transport mode                                    │
   │                                                                    │
   │  4. Start BLE advertising:                                        │
   │     Service UUID: 0000FDA0-0000-1000-8000-00805F9B34FB            │
   │     Advertisement data:                                           │
   │     ┌──────────────────────────────────────────────┐              │
   │     │ Name: "P|{roomId}|{roomName}"               │              │
   │     │ ManufData: {hostId, hostName, wifiAvail...} │              │
   │     └──────────────────────────────────────────────┘              │
   │                                                                    │
   │  5. Start HTTP server (if Wi-Fi):                                 │
   │     Port: 8080                                                    │
   │     Endpoints:                                                    │
   │     • GET  /room           → Room state                           │
   │     • POST /join           → Join request                         │
   │     • GET  /files          → File index                           │
   │     • GET  /files/{id}     → Download file                        │
   │     • WS   /ws             → Real-time events                     │
   │                                                                    │
   │  6. Initialize room state:                                        │
   │     { peers: [], files: [], createdAt: timestamp }                │
   │                                                                    │
   └────────────────────────────────────────────────────────────────────┘


PHASE 2: DISCOVERY (GUEST)
────────────────────────────────────────────────────────────────────────────────

   GUEST DEVICE
   ┌────────────────────────────────────────────────────────────────────┐
   │                                                                    │
   │  1. User action: "Find Rooms"                                     │
   │                                                                    │
   │  2. Request BLE permissions (if needed)                           │
   │                                                                    │
   │  3. Start BLE scanning:                                           │
   │     Filter: Service UUID 0000FDA0-...                             │
   │     Options: { allowDuplicates: true }                            │
   │                                                                    │
   │  4. For each discovered device:                                   │
   │     • Parse advertisement name                                    │
   │     • Extract: roomId, roomName                                   │
   │     • Parse manufacturer data (if present)                        │
   │     • Record RSSI for sorting                                     │
   │     • Add to discovered rooms list                                │
   │                                                                    │
   │  5. Maintain room list:                                           │
   │     • Update last_seen timestamp                                  │
   │     • Remove rooms not seen for 15s                               │
   │     • Sort by RSSI (signal strength)                              │
   │                                                                    │
   │  6. Display to user:                                              │
   │     ┌──────────────────────────────────────┐                      │
   │     │ 📶 DJ Set @ Club          ████ Host1 │                      │
   │     │ 📶 Chill Zone             ███  Host2 │                      │
   │     │ 📶 Live Recording         ██   Host3 │                      │
   │     └──────────────────────────────────────┘                      │
   │                                                                    │
   └────────────────────────────────────────────────────────────────────┘


PHASE 3: JOIN (HANDSHAKE)
────────────────────────────────────────────────────────────────────────────────

   GUEST                                         HOST
   ─────                                         ────

   ┌──────────────────────┐
   │ 1. Connect via BLE   │
   │    GATT              │
   └──────────┬───────────┘
              │
              │  BLE GATT Connection
              │──────────────────────────────────►  ┌───────────────────────┐
              │                                     │ 2. Accept connection  │
              │                                     └───────────────────────┘
              │
   ┌──────────────────────┐
   │ 3. Write to JOIN     │
   │    characteristic:   │
   │    {peerId, peerName}│
   └──────────┬───────────┘
              │
              │  JOIN_REQUEST
              │──────────────────────────────────►  ┌───────────────────────┐
              │                                     │ 4. Validate request   │
              │                                     │ 5. Generate token     │
              │                                     │ 6. Add to pending     │
              │                                     └───────────┬───────────┘
              │                                                 │
              │                                     JOIN_RESPONSE
              │◄──────────────────────────────────  {success, token,
              │                                      hostAddress}
              │
   ┌──────────────────────┐
   │ 7. Disconnect BLE    │
   │    (handshake done)  │
   └──────────────────────┘
              │
              │  [If Wi-Fi available]
              │
              │  HTTP POST /join
              │  { peerId, peerName, sessionToken }
              │──────────────────────────────────►  ┌───────────────────────┐
              │                                     │ 8. Validate token     │
              │                                     │ 9. Add to peers[]     │
              │                                     │ 10. Broadcast:        │
              │                                     │     PEER_JOINED       │
              │                                     └───────────┬───────────┘
              │                                                 │
              │                                     200 OK
              │◄──────────────────────────────────  { roomState }
              │
   ┌──────────────────────┐
   │ 11. Connect WebSocket│
   │     for real-time    │
   └──────────────────────┘
              │
              │  WS /ws?token={token}
              │══════════════════════════════════►  (bidirectional)


PHASE 4: METADATA SYNC
────────────────────────────────────────────────────────────────────────────────

   GUEST                                         HOST
   ─────                                         ────

   ┌──────────────────────┐
   │ 1. Select files to   │
   │    share             │
   └──────────┬───────────┘
              │
              │  WS: SHARE_FILES
              │  { files: [
              │      { id, title, artist, duration, size, format }
              │    ]
              │  }
              │══════════════════════════════════►  ┌───────────────────────┐
              │                                     │ 2. Add to room index  │
              │                                     │ 3. Associate with     │
              │                                     │    peer address       │
              │                                     └───────────┬───────────┘
              │                                                 │
              │                                     WS: INDEX_UPDATED
              │◄════════════════════════════════════(broadcast to all)
              │                                     { files: [...] }


PHASE 5: FILE TRANSFER (P2P)
────────────────────────────────────────────────────────────────────────────────

   GUEST A                  HOST                 GUEST B
   (Requester)              ────                 (Owner)
   ───────────

   ┌──────────────────────┐
   │ 1. Select file       │
   │    owned by Guest B  │
   └──────────┬───────────┘
              │
              │  WS: REQUEST_FILE
              │  { fileId, ownerId }
              │══════════════════════►  ┌────────────────────────┐
              │                         │ 2. Lookup owner        │
              │                         │    in peers[]          │
              │                         └────────────┬───────────┘
              │                                      │
              │                         WS: FILE_AVAILABLE
              │◄═══════════════════════ { fileId, ownerAddress }
              │
              │
              │  ┌─────────────────────────────────────────────┐
              │  │           DIRECT P2P CONNECTION             │
              │  └─────────────────────────────────────────────┘
              │
              │  HTTP GET /files/{fileId}
              │  Headers: X-Session-Token: {token}
              │────────────────────────────────────────────────►
              │                                                  ┌──────────────┐
              │                                                  │ 3. Validate  │
              │                                                  │ 4. Open file │
              │                                                  │ 5. Stream    │
              │                                                  └──────┬───────┘
              │                                                         │
              │  200 OK                                                 │
              │  Content-Type: audio/mpeg                               │
              │  Content-Length: 5242880                                │
              │  Transfer-Encoding: chunked                             │
              │◄────────────────────────────────────────────────────────
              │
   ┌──────────────────────┐
   │ 6. Receive chunks    │
   │ 7. Write to disk     │
   │ 8. Update progress   │
   └──────────┬───────────┘
              │
              │  WS: TRANSFER_COMPLETED
              │  { fileId, transferId }
              │══════════════════════►  (notify room)
```

---

## 5. Formati dei Messaggi

### 5.1 BLE Advertisement

```typescript
// Device Name Format (max 29 chars)
"P|{roomId:8}|{roomName:encoded}"

// Example
"P|a1b2c3d4|DJ%20Set%20Night"

// Manufacturer Data (JSON, base64 encoded)
{
  "v": 1,                      // Protocol version
  "hId": "host-uuid",          // Host ID
  "hN": "DJ Marco",            // Host Name
  "wifi": true,                // Wi-Fi available
  "addr": "192.168.1.42:8080"  // Host address
}
```

### 5.2 WebSocket Messages

```typescript
// Base Message
interface BaseMessage {
  type: MessageType;
  timestamp: number;
  senderId: string;
}

// Message Types
enum MessageType {
  JOIN_ROOM = 'join_room',
  LEAVE_ROOM = 'leave_room',
  ROOM_STATE = 'room_state',
  PEER_JOINED = 'peer_joined',
  PEER_LEFT = 'peer_left',
  ROOM_CLOSING = 'room_closing',
  SHARE_FILES = 'share_files',
  UNSHARE_FILES = 'unshare_files',
  INDEX_UPDATED = 'index_updated',
  REQUEST_FILE = 'request_file',
  FILE_AVAILABLE = 'file_available',
  TRANSFER_STARTED = 'transfer_started',
  TRANSFER_PROGRESS = 'transfer_progress',
  TRANSFER_COMPLETED = 'transfer_completed',
  TRANSFER_FAILED = 'transfer_failed',
  PING = 'ping',
  PONG = 'pong',
  ERROR = 'error',
}

// Example: Share Files
{
  "type": "share_files",
  "timestamp": 1703270400000,
  "senderId": "peer-uuid",
  "files": [
    {
      "fileId": "file-uuid",
      "fileName": "track.mp3",
      "title": "My Track",
      "artist": "Artist Name",
      "duration": 234,
      "sizeBytes": 5242880,
      "format": "mp3",
      "checksum": "sha256:abc..."
    }
  ]
}

// Example: Request File
{
  "type": "request_file",
  "timestamp": 1703270400000,
  "senderId": "requester-uuid",
  "fileId": "file-uuid",
  "requesterId": "requester-uuid"
}

// Example: File Available
{
  "type": "file_available",
  "timestamp": 1703270400000,
  "senderId": "host-uuid",
  "fileId": "file-uuid",
  "ownerAddress": "192.168.1.50:8080",
  "transportMode": "wifi_lan"
}
```

---

## 6. Gestione degli Edge Case

### 6.1 Host Disconnection

```
SCENARIO: L'host chiude l'app o perde connessione
────────────────────────────────────────────────────

DETECTION:
• WebSocket disconnection timeout: 5s
• Missing PONG response: 3 retries

HANDLING:
1. Guests ricevono: WS connection closed
2. UI mostra: "Room closed"
3. Auto-cleanup dopo 10s
4. Trasferimenti in corso: CANCELLED
5. File parziali: eliminati

RECOVERY:
• Guest può cercare nuove room
• Nessun dato perso (file già completati salvati)
```

### 6.2 Network Drop

```
SCENARIO: Wi-Fi temporaneamente non disponibile
────────────────────────────────────────────────────

DETECTION:
• HTTP request timeout: 10s
• WebSocket ping timeout: 5s

HANDLING:
1. Trasferimenti: PAUSED
2. UI mostra: "Connessione persa"
3. Retry automatico ogni 5s (max 3 tentativi)

RECOVERY (se rete torna):
1. Riconnessione WebSocket
2. Re-sync stato room
3. Resume trasferimenti (se supportato)

FALLBACK (se rete non torna):
1. Switch a BLE-only mode (se abilitato)
2. Trasferimenti file grandi: CANCELLED
3. Solo metadati via BLE
```

### 6.3 Partial Transfer

```
SCENARIO: Trasferimento interrotto a metà
────────────────────────────────────────────────────

CAUSES:
• Network drop
• Peer disconnection
• App backgrounded (iOS)

HANDLING:
1. File parziale salvato con estensione .partial
2. Metadata preservati
3. Progress salvato

RECOVERY:
• Se peer disponibile: resume con Range header
• Se peer non disponibile: retry completo o cancel

CLEANUP:
• File .partial eliminati dopo 24h
• O quando utente cancella manualmente
```

---

## 7. Limitazioni di Piattaforma

### 7.1 iOS

```
┌─────────────────────────────────────────────────────────────────┐
│                         iOS LIMITATIONS                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  BLE:                                                           │
│  ⚠️ Background scanning: molto limitato                        │
│  ⚠️ Background advertising: solo con permission speciale       │
│  ✓ Foreground: funziona bene                                   │
│                                                                 │
│  Network:                                                       │
│  ⚠️ Local network permission richiesta (iOS 14+)               │
│  ⚠️ Background transfers: limitati                             │
│  ✓ Foreground: funziona bene                                   │
│                                                                 │
│  Best Practices:                                                │
│  • Mantieni app in foreground durante trasferimenti            │
│  • Usa screen wake lock                                        │
│  • Mostra progress prominente                                  │
│  • Avvisa utente se va in background                           │
│                                                                 │
│  Required Info.plist keys:                                      │
│  • NSBluetoothAlwaysUsageDescription                           │
│  • NSBluetoothPeripheralUsageDescription                       │
│  • NSLocalNetworkUsageDescription                              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 7.2 Android

```
┌─────────────────────────────────────────────────────────────────┐
│                       ANDROID LIMITATIONS                        │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  BLE:                                                           │
│  ⚠️ Requires location permission (Android 10+)                 │
│  ⚠️ BLUETOOTH_SCAN, BLUETOOTH_CONNECT (Android 12+)            │
│  ✓ Background: più permissivo di iOS                           │
│  ✓ Throughput: generalmente superiore                          │
│                                                                 │
│  Network:                                                       │
│  ✓ Local network: nessuna permission speciale                  │
│  ⚠️ Background: Doze mode può interferire                      │
│                                                                 │
│  Best Practices:                                                │
│  • Request all permissions upfront                             │
│  • Use foreground service for long transfers                   │
│  • Handle Doze mode properly                                   │
│                                                                 │
│  Required permissions:                                          │
│  • BLUETOOTH                                                    │
│  • BLUETOOTH_ADMIN                                              │
│  • BLUETOOTH_SCAN                                               │
│  • BLUETOOTH_CONNECT                                            │
│  • BLUETOOTH_ADVERTISE                                          │
│  • ACCESS_FINE_LOCATION                                         │
│  • ACCESS_WIFI_STATE                                            │
│  • READ_MEDIA_AUDIO (Android 13+)                              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 📚 Riferimenti

- [Bluetooth Low Energy Core Specification](https://www.bluetooth.com/specifications/specs/core-specification/)
- [React Native BLE PLX](https://github.com/dotintent/react-native-ble-plx)
- [Expo File System](https://docs.expo.dev/versions/latest/sdk/filesystem/)
- [Apple Local Network Privacy](https://developer.apple.com/documentation/bundleresources/information_property_list/nslocalnetworkusagedescription)

---

*Documento generato per PANDEMIC v1.0.0*

